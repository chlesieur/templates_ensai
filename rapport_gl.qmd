...
Une fois ce document vidé de son contenu, il en restera le « squelette » qui pourra servir comme base de départ à la rédaction de tout rapport
...
Autre exemple où on « plante » la graine numéro *n* du générateur de nombres aléatoires. La graine (« seed » en anglais) permet d’obtenir une reproductibilité des résultats quand l’on fait appel à un processus aléatoire. Pour cela, il faut appeler à chaque fois la même graine (c’est-à-dire le même numéro de graine).
...
En français[^Canada], les signes de ponctuation dits doubles car composés de deux éléments (« : », « ; », « ? », « ! ») sont précédés par une espace. En anglais, pas d’espace avant ces signes de ponctuation.
...
Pour faire une note de bas de page, il faut mettre à l’endroit souhaité un nom en chiffres ou en lettres précédé d’un accent circonflexe entre crochets comme ceci : « `[^6]` » et cela donne [^6] puis dans un paragraphe suivant, on fait le rappel de la note de la même façon avec le texte associé après deux-points.

[^6]: Voici le contenu de la note de bas de page qui apparaîtra en bas de la même page.
