...
Il faut une espace[^espace] entre les croisillons et le titre choisi de la section.
...
Un document Quarto sur R permet de mêler du texte et du code exécutable pour tout regrouper dans un seul document finalisé, comme ici un rapport en format PDF. Pour en savoir plus sur Quarto : [site officiel de Quarto](https://quarto.org)[^1] (en anglais).
...
Voir [dans les annexes](#starwars) à quoi ressemble le contenu de la table.

\pagebreak

### Un dernier exemple avec mix entre texte et code intégré

Autre exemple où je « plante » la graine numéro *n* du générateur de nombres aléatoires. La graine (« seed » en anglais) permet d’obtenir une reproductibilité des résultats quand l’on fait appel à un processus aléatoire. Pour cela, il faut appeler à chaque fois la même graine (c’est-à-dire le même numéro de graine).
...
Pour avoir, en dehors d’un *chunk*, une phrase reprenant un résultat issu de code dans R, j’écris, entre deux accents graves (un au début et un à la fin), « `r` » (pour dire que je veux ce code dans R) suivi d’une espace et enfin du code R.\
...
En français (sauf français du Canada), les signes de ponctuation composés d’au moins deux éléments (« : », « ; », « ? », etc.) sont précédés par une espace (à l’exception des points de suspension et du guillemet ouvrant si celui-ci suit une apostrophe). En anglais, pas d’espace avant ces signes de ponctuation sauf pour le guillemet ouvrant.

Pour les espaces avant les signes de ponctuation, le document Quarto sur R gère cela au moment où le PDF est créé. Ainsi, selon, si dans l’en-tête YAML, on a indiqué « lang: fr » ou « lang: en », les espaces seront respectivement automatiquement rajoutées ou supprimées.
...
Dans les titres d’ouvrage, les titres en français se limitent généralement à une majuscule au premier mot quand les titres en anglais ont une majuscule à chaque mot important.
...
Les abréviations en français se terminent généralement (pas toujours) par un point. Les abréviations anglaises souvent sans point (et il y a une différence entre les conventions américaines et britanniques). Ne pas hésiter à consulter des listes d’abréviations courantes pour ne pas se tromper.
...
Le premier résultat est `r v` et le second est `r w`. Comme on le voit ici, R n’utilise pas les séparateurs de milliers, mais si vous devez écrire un nombre avec beaucoup de chiffres, le séparateur de milliers est une espace en français et une virgule en anglais, respectivement : « 61 425 » et « 61,425 ».
...
Si on passe à la ligne sans sauter de ligne et sans les deux espaces à la ligne précédente, cela écrira tout à la suite. Comme ce qui est écrit ici.

## Renvois {#renvois}

### Notes de bas de page

Pour faire une note de bas de page, il faut mettre à l’endroit souhaité un nom en chiffres ou en lettres précédé d’un accent circonflexe entre crochets comme ceci : « `[^5]` » et cela donne [^3] puis dans un paragraphe suivant, on fait le rappel de la note de la même façon avec le texte associé après deux-points.

[^3]: Voici le contenu de la note de bas de page qui apparaîtra en bas de la même page.

Test avec « `[^a]` »[^4].

[^4]: Ça marche aussi avec une lettre.

Test avec « `[^bidule]` »[^5].

[^5]: Ça marche aussi avec une note qui porte un nom.

Test avec « `[^fan2foot]` »[^6].

[^6]: Ça marche aussi avec une note qui porte un nom qui mixe chiffres et lettres.
