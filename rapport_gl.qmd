...
<!--La présentation de la page de garde est codée ci-dessous en LaTeX ; page officielle de ce langage de programmation : https://www.latex-project.org/ (en anglais).-->
...
Points typographiques : pour qu’un symbole (accent grave ou autre) ne soit pas compris par R comme du code dans du texte, on doit l’entourer de chaque côté par deux accents graves (cf. le code de la section précédente : si on est actuellement dans le mode **Visual**, pour regarder ce code, cliquer sur le bouton **Source** de RStudio ou Ctrl+Maj+F4).
...
- si on tape « `` ` `` `Machin` `` ` `` » dans le code, cela affiche « `Machin` » ;

- alors que si on tape « `Machin` », ça affiche « Machin ».
