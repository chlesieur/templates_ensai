...
<!--La présentation de la page de titre est codée ci-dessous en LaTeX qui est un langage supporté par les documents Quarto sur R ; sauf à vouloir innover la présentation, vous n’avez pas à apprendre le langage LaTeX. Page officielle de ce langage de programmation : https://www.latex-project.org/ (en anglais).-->
...
Si la portion que l’on souhaite afficher comporte trois accents graves (comme pour le début ou la fin d’un *chunk*), on doit entourer cette portion avec quatre accents graves de chaque côté (cf. le début de [la section « Mix entre texte et code intégré »](#mix)).
